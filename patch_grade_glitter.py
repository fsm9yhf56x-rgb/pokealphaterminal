#!/usr/bin/env python3
"""Grade 10 badges: meme design que la barre — sweep + glitter"""
from pathlib import Path

f = Path('src/components/features/portfolio/Holdings.tsx')
s = f.read_text('utf-8')

# 1. Ajouter le generateur de glitter pour les badges dans le useEffect existant
old_effect = """    els.forEach(el => {
      if (el.childNodes.length > 0) return
      const anims = ['gl1','gl2','gl3','gl4']
      for (let i = 0; i < 2000; i++) {"""
new_effect = """    els.forEach(el => {
      if (el.childNodes.length > 0) return
      const anims = ['gl1','gl2','gl3','gl4']
      for (let i = 0; i < 2000; i++) {"""
# Keep master bar as is, add badge glitter after
old_effect_end = """      el.appendChild(d)
    }
    })
  })"""
new_effect_end = """      el.appendChild(d)
    }
    })
    ...
    badges.forEach(el => {
      ...
      for (let i = 0; i < 80; i++) {
        ...
        const top = (-1 + Math.random() * (el.clientHeight + 2)).toFixed(0)
...
assert old_effect_end in s, "CIBLE EFFECT END"
s = s.replace(old_effect_end, new_effect_end, 1)
print('  > badge glitter generator')

# 2. SHELF badge — or continu + sweep + glitter container
...
new_shelf = """return <div style={{ position:'absolute', bottom:'28px', right:'4px', zIndex:3, background:gn>=10?'linear-gradient(90deg,#C9A84C,#D4AF37,#E8D48B,#D4AF37,#C9A84C)':bg, color:gn>=10?'#5C4A12':fg, fontSize:'8px', fontWeight:800, padding:'3px 7px', borderRadius:'5px', fontFamily:'var(--font-data)', boxShadow:gn>=10?'0 2px 8px rgba(212,175,55,.3)':sh, letterSpacing:'.03em', overflow:'visible', border:gn>=10?'1px solid rgba(212,175,55,.3)':'none' }}>
                                    {gn>=10&&<>
                                      <div style={{ position:'absolute', inset:0, overflow:'hidden', borderRadius:'5px' }}>
                                        <div style={{ position:'absolute', top:0, width:'30px', height:'100%', background:'linear-gradient(90deg,transparent,rgba(255,255,250,.7),transparent)', animation:'masterSweep 6s ease-in-out infinite' }}/>
                                        <div style={{ position:'absolute', top:0, width:'20px', height:'100%', background:'linear-gradient(90deg,transparent,rgba(255,255,250,.45),transparent)', animation:'masterSweep 6s 3s ease-in-out infinite' }}/>
                                      </div>
                                      <div className='badge-glitter-container' style={{ position:'absolute', inset:'-1px 0', pointerEvents:'none' }}/>
                                    </>}
                                    <span style={{ position:'relative', zIndex:1 }}>{card.condition}</span>
                                  </div>"""
...
new_grid = """return <span style={{ fontSize:binderCols>=7?'7px':'8px', fontWeight:800, background:gn3>=10?'linear-gradient(90deg,#C9A84C,#D4AF37,#E8D48B,#D4AF37,#C9A84C)':bg3, color:gn3>=10?'#5C4A12':fg3, padding:'2px 6px', borderRadius:'4px', fontFamily:'var(--font-data)', boxShadow:gn3>=10?'0 2px 6px rgba(212,175,55,.25)':sh3, letterSpacing:'.03em', marginLeft:'2px', overflow:'visible', position:'relative', border:gn3>=10?'1px solid rgba(212,175,55,.3)':'none', display:'inline-flex', alignItems:'center' }}>
                                {gn3>=10&&<>
                                  <span style={{ position:'absolute', inset:0, overflow:'hidden', borderRadius:'4px' }}>
                                    <span style={{ position:'absolute', top:0, width:'30px', height:'100%', background:'linear-gradient(90deg,transparent,rgba(255,255,250,.7),transparent)', animation:'masterSweep 6s ease-in-out infinite' }}/>
                                    <span style={{ position:'absolute', top:0, width:'20px', height:'100%', background:'linear-gradient(90deg,transparent,rgba(255,255,250,.45),transparent)', animation:'masterSweep 6s 3s ease-in-out infinite' }}/>
                                  </span>
                                  <span className='badge-glitter-container' style={{ position:'absolute', inset:'-1px 0', pointerEvents:'none' }}/>
                                </>}
                                <span style={{ position:'relative', zIndex:1 }}>{card.condition}</span>
                              </span>"""
