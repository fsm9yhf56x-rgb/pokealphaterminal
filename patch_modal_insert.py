#!/usr/bin/env python3
"""Insert le modal avant WELCOME — cherche le vrai commentaire"""
...
# Trouver le commentaire WELCOME exact
idx = s.find('WELCOME')
if idx < 0:
    print('WELCOME non trouve')
    exit(1)

# Reculer pour trouver le debut du commentaire {/*
start = s.rfind('{/*', 0, idx)
welcome_line = s[start:s.find('\n', idx)]
print(f'  > found: {repr(welcome_line[:50])}')

if "{/* ADD SET MODAL */}" in s:
    print('  > modal deja present')
    exit(0)
...
# Inserer avant le commentaire WELCOME (quel que soit son format)
s = s[:start] + modal + s[start:]
print('  > modal insere')
