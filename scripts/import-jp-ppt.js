/**
 * Import du referentiel JP depuis PokemonPriceTracker (PPT) vers Neon + R2.
 *
 * Pour chaque set JP PPT:
 *   - cree/upsert le set dans tcg_sets (id = jp-{slug}, source='ppt')
 *   - pour chaque carte: cree tcg_cards (id = jp-{tcgPlayerId}, source='ppt')
 *   - telecharge image TCGPlayer -> sharp coins arrondis webp -> R2 jp/{slug}/{localId}.jpg
 *
 * Convention id: jp-{tcgPlayerId} (unique + stable). local_id = cardNumber sans /total,
 * fallback tcgPlayerId si absent (vintage sans numero).
 *
 * Usage:
 *   node scripts/import-jp-ppt.js --set "SV2a: Pokemon Card 151"   (1 set pilote)
 *   node scripts/import-jp-ppt.js --all                            (442 sets, ~65k credits)
 *   node scripts/import-jp-ppt.js --set "..." --dry-run            (aucune ecriture)
 *   node scripts/import-jp-ppt.js --set "..." --no-images          (skip R2)
 *
 * Credits: PPT /v2/cards = 2 cr/carte. Telechargement image (TCGPlayer CDN) = GRATUIT.
 */
...
async function fetchSetCards(setName) {
  let all = [], offset = 0;
  while (true) {
    const url = `${PPT_BASE}/cards?set=${encodeURIComponent(setName)}&language=japanese&fetchAllInSet=true&limit=200&offset=${offset}`;
...
async function importSet(setName, setMeta) {
  ...
  if (DRY) {
    ...
    for (const c of cards.slice(0, 3)) {
      console.log(`     jp-${c.tcgPlayerId} | local=${localIdFrom(c)} | "${cleanName(c.name)}" | img=${c.imageCdnUrl ? 'oui' : 'non'}`);
    }
  ...
  await sql`
    ...
    VALUES (${setId}, ${setName}, 'JP', ${setMeta?.cardCount || cards.length}, ${setMeta?.releaseDate || null}, 'ppt', true)
  ...
  for (let i = 0; i < cards.length; i += BATCH) {
    ...
    for (const c of chunk) {
      ...
      await sql`
        ...
        VALUES (${cardId}, ${setId}, ${localId}, ${cleanName(c.name)}, 'JP', ${c.rarity || null}, ${c.cardType || null}, ${c.hp || null}, ${hasImage}, true, 'ppt', ${c.imageCdnUrl || null})
...
(async () => {
  ...
  if (ONE_SET) {
    ...
    let offset = 0;
    while (true) {
      const r = await fetch(`${PPT_BASE}/sets?language=japanese&limit=100&offset=${offset}`, { headers: { Authorization: 'Bearer ' + KEY } });
      const j = await r.json();
