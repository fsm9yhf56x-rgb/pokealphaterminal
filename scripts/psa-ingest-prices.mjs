// scripts/psa-ingest-prices.mjs
// Capture les annonces PSA sur eBay FR -> staging immuable psa_price_raw.
// Meme moteur que ccc-ingest-prices.mjs. PSA n'a ni Black ni Gold : notes 1-10 (+ demi).
// N'ECRIT RIEN dans price_matrix : staging. Le matcher generalise vient apres.
import fs from 'fs';
...
await sql.query(`CREATE TABLE IF NOT EXISTS psa_price_raw (
...
await sql.query(`UPDATE psa_price_raw SET first_seen=fetched_at WHERE first_seen IS NULL`);
await sql.query(`UPDATE psa_price_raw SET last_seen=fetched_at WHERE last_seen IS NULL`);
...
function parseGrade(raw){
  ...
  // PSA : pas de Black/Gold. Notes 1-10 + demi-grades (9.5 etc.).
  let m = low.match(/psa[^0-9a-z]{0,10}(10|[1-9](?:\.5)?)\b/)
       || low.match(/\b(10|[1-9](?:\.5)?)[^0-9a-z]{0,6}psa\b/)
       || low.match(/grad[eé]+e?\s*psa[^0-9]{0,10}(10|[1-9](?:\.5)?)\b/);
  const num = m ? parseFloat(m[1]) : null;
  if(num == null) return { num:null, label:null, tier:null };
  const label = String(num);
  return { num, label, tier:`PSA_${String(num).replace('.', '_')}` };
}
...
// -- Capture paginee, multi-requetes ------------------------------------------
const QUERIES = ['PSA pokemon français','pokemon PSA 10 fr','carte pokemon gradée PSA','PSA pokémon française','PSA pokemon edition 1 fr'];
let captured = 0;
const seen = new Set();
for(const Q of QUERIES){
  for(const off of [0,200,400,600,800]){
    ...
    for(const it of items){
      ...
      await sql.query(`INSERT INTO psa_price_raw
...
const s = (await q(`SELECT
  ...
  FROM psa_price_raw`))[0];
console.log(`=== psa_price_raw (capture: ${captured}) ===`);
console.log(s);
console.log('=== Repartition par grade (FR, matchable) ===');
for(const r of await q(`SELECT tier, COUNT(*)::int n, ROUND(AVG(price))::int prix_moy_ask
  FROM psa_price_raw WHERE NOT excluded AND NOT is_lot AND lang='FR'
