...
export async function POST(request: Request) {
  ...
  if (setsToSync.length === 0) {
    ...
    // Chercher d'abord le setId exact, sinon le setId nettoyé
    const resolved = rawSets.map(s => setMapping[s] ? s : cleanSet(s))
    const uniqueSets = [...new Set(resolved)]
    ...
    try {
      ...
      for (const card of cards) {
        ...
        await supabase.from('prices').upsert({
          ...
          top_price: ebay?.avg && tcg?.avg ? Math.round((ebay.avg * 0.5 + tcg.avg * 0.5) * 100) / 100 : ebay?.avg || tcg?.avg || card.topPrice || null, total_sales: card.totalSaleCount || null,
