...
const RARE_RX = /(alt art|secret|rainbow|special|illustration|hyper|gold|1st|edition|holo rare|full art|amazing|shiny)/i
...
export async function GET(req: Request) {
  ...
  const isRare = !isGraded && RARE_RX.test(rarity)
  const graderCol = GRADER_COLOR[grader] || '#C0392B'
  const goldFrame = isGraded || isPremium
  ...
  const Chips = () => (
    <div style={{ display: 'flex', alignItems: 'center' }}>
      ...
      {isRare ? (<div style={{ display: 'flex', fontSize: isWide ? '20px' : '24px', fontWeight: 700, color: '#0A0A14', background: 'linear-gradient(135deg,#F5D76E,#C9A227)', borderRadius: '999px', padding: isWide ? '8px 16px' : '10px 20px', marginRight: '12px', letterSpacing: '1px', textTransform: 'uppercase' }}>{rarity}</div>) : rarity ? (<div style={{ display: 'flex', fontSize: isWide ? '20px' : '24px', fontWeight: 700, color: 'rgba(255,255,255,0.85)', background: 'rgba(255,255,255,0.06)', borderRadius: '999px', padding: isWide ? '9px 16px' : '11px 20px', marginRight: '12px', letterSpacing: '1px', textTransform: 'uppercase' }}>{rarity}</div>) : null}
  ...
  try {
    ...
    // Carte + badge (commun)
    const CardBlock = () => (
      <div style={{ display: 'flex', position: 'relative', borderRadius: '26px', padding: goldFrame ? '5px' : '0', background: goldFrame ? 'linear-gradient(135deg,#F5D76E,#C9A227,#F5D76E)' : 'transparent', boxShadow: goldFrame ? '0 26px 70px rgba(201,162,39,0.35)' : '0 34px 90px rgba(0,0,0,0.6)' }}>
        {imgData ? (
            ...
            <div style={{ display: 'flex', flex: 1, flexDirection: 'column', justifyContent: 'space-between', padding: '52px 56px 44px 30px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                {hasPrice ? (
                  <div style={{ display: 'flex', flexDirection: 'column', marginTop: '26px' }}>
                    ...
                    <div style={{ display: 'flex', fontSize: '72px', fontWeight: 700, color: isPremium ? '#F5D76E' : '#fff', letterSpacing: '-2px' }}>{priceLabel}</div>
                  </div>
                ) : (
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ display: 'flex', fontSize: '18px', color: 'rgba(255,255,255,0.5)', marginTop: '4px' }}>Scanne pour ta collection Pokémon</div>
                </div>
        ...
        <div style={{ width: `${W}px`, height: `${H}px`, display: 'flex', flexDirection: 'column', background: '#0A0A14', fontFamily: FF, position: 'relative' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 72px' }}>
            ...
            {hasPrice ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: '32px' }}>
                ...
                <div style={{ display: 'flex', fontSize: format === 'post' ? '96px' : '110px', fontWeight: 700, color: isPremium ? '#F5D76E' : '#fff', letterSpacing: '-3px', marginTop: '2px' }}>{priceLabel}</div>
              </div>
            ) : (
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '36px 60px 56px', marginTop: '34px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ display: 'flex', fontSize: '22px', color: 'rgba(255,255,255,0.5)', marginTop: '6px' }}>Scanne pour ta collection Pokémon</div>
            </div>
