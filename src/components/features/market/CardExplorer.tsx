...
function mockSales(card:Card){return[
...
export function CardExplorer(){
  ...
  const [sort,setSort]=useState<'name'|'price'|'change'|'vol'|'psa'>('vol')
  ...
  const activeFilters=[filterSet,filterRarity,filterEra,filterPriceMin,filterPriceMax,filterGrade,filterTrend].filter(f=>f&&f!=='all').length

  const resetFilters=()=>{setSearch('');setFilterSet('all');setFilterRarity('all');setFilterEra('all');setFilterPriceMin('');setFilterPriceMax('');setFilterGrade('all');setFilterTrend('all')}

  const filtered=useMemo(()=>{
    let list=[...CARDS]
    if(search){const q=search.toLowerCase();list=list.filter(c=>c.name.toLowerCase().includes(q)||c.set.toLowerCase().includes(q)||c.rarity.toLowerCase().includes(q)||c.number.includes(q))}
    if(filterSet!=='all')list=list.filter(c=>c.set===filterSet)
    if(filterRarity!=='all')list=list.filter(c=>c.rarity===filterRarity)
    if(filterEra!=='all'){
      if(filterEra==='vintage')list=list.filter(c=>VINTAGE_SETS.includes(c.set))
      if(filterEra==='modern')list=list.filter(c=>!VINTAGE_SETS.includes(c.set))
    }
    if(filterPriceMin)list=list.filter(c=>c.price>=Number(filterPriceMin))
    if(filterPriceMax)list=list.filter(c=>c.price<=Number(filterPriceMax))
    if(filterGrade!=='all'){
      if(filterGrade==='psa10+')list=list.filter(c=>c.psa10>=1000)
      if(filterGrade==='premium')list=list.filter(c=>(c.psa10/c.price)>=2)
    }
    if(filterTrend!=='all'){
      if(filterTrend==='up')list=list.filter(c=>c.change>0)
      if(filterTrend==='down')list=list.filter(c=>c.change<0)
      if(filterTrend==='hot')list=list.filter(c=>c.change>15)
      if(filterTrend==='stable')list=list.filter(c=>Math.abs(c.change)<5)
    }
    list.sort((a,b)=>{
      if(sort==='name')return a.name.localeCompare(b.name)
      if(sort==='price')return b.price-a.price
      if(sort==='change')return b.change-a.change
      if(sort==='psa')return b.psa10-a.psa10
      return b.vol-a.vol
    })
    return list
  },[search,filterSet,filterRarity,filterEra,filterPriceMin,filterPriceMax,filterGrade,filterTrend,sort])

  const curIdx=filtered.findIndex(c=>c.name===sel)
  const nav=(dir:-1|1)=>{const next=curIdx+dir;if(next>=0&&next<filtered.length){setSel(filtered[next].name);setPeriod('1M')}}
  ...
  return(
    <>
      <style>{`
        ...
        .f-label{font-size:9px;color:#AAA;font-family:var(--font-display);font-weight:500;letter-spacing:.05em;text-transform:uppercase;margin-bottom:3px;padding-left:2px}
        .f-row{display:flex;gap:3px;flex-wrap:wrap}
        .f-chip{padding:3px 8px;border-radius:5px;border:1px solid #EBEBEB;background:#fff;font-size:10px;color:#888;cursor:pointer;font-family:var(--font-display);transition:all .1s;white-space:nowrap}
        .f-chip:hover{border-color:#C7C7CC;color:#111}
        .f-chip.on{background:#111;color:#fff;border-color:#111}
        .f-price{width:60px;padding:4px 6px;border-radius:5px;border:1px solid #EBEBEB;font-size:10px;font-family:var(--font-data);outline:none;text-align:center}
        .f-price:focus{border-color:#E03020}
        .f-price::placeholder{color:#CCC}
        .f-toggle{display:flex;align-items:center;gap:6px;padding:6px 10px;border-radius:7px;border:1px solid #EBEBEB;cursor:pointer;transition:all .12s;font-size:11px;font-family:var(--font-display);color:#888;background:#fff}
        .f-toggle:hover{border-color:#C7C7CC;color:#111}
        .f-toggle.on{background:#FFF5F4;border-color:#FFD8D0;color:#E03020}
        .f-reset{padding:4px 10px;border-radius:5px;border:1px solid #FFD8D0;background:#FFF5F4;font-size:10px;color:#E03020;cursor:pointer;font-family:var(--font-display);font-weight:500}
        .f-count{display:inline-flex;align-items:center;justify-content:center;width:16px;height:16px;border-radius:50%;background:#E03020;color:#fff;font-size:8px;font-weight:700}
      `}</style>
        ...
        <div style={{width:320,borderRight:'1px solid #EBEBEB',display:'flex',flexDirection:'column',flexShrink:0,background:'#fff'}}>
          ...
          <div style={{padding:'12px 12px 8px'}}>
            <div style={{display:'flex',alignItems:'center',gap:6,background:'#F5F5F7',border:'1px solid #EBEBEB',borderRadius:8,padding:'7px 10px'}}>
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Nom, set, raret\u00e9, num\u00e9ro..."
          ...
          <div style={{padding:'0 12px 6px',display:'flex',alignItems:'center',gap:6}}>
            <button className={'f-toggle'+(showFilters?' on':'')} onClick={()=>setShowFilters(f=>!f)}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M22 3H2l8 9.46V19l4 2v-8.54L22 3z"/></svg>
              Filtres
              {activeFilters>0&&<span className="f-count">{activeFilters}</span>}
            </button>
            <div style={{flex:1}}/>
            <div style={{display:'flex',gap:2}}>
              {(['vol','price','change','psa','name'] as const).map(s=>(
                <button key={s} className={'sort-btn'+(sort===s?' on':'')} onClick={()=>setSort(s)}>
                  {s==='vol'?'Vol':s==='price'?'Prix':s==='change'?'%':s==='psa'?'PSA':'A-Z'}
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Filters panel */}
          {showFilters&&(
            <div style={{padding:'0 12px 8px',borderBottom:'1px solid #F5F5F5',marginBottom:4}}>
              <div className="f-group">
                <div className="f-label">{'\u00c9'}re</div>
                <div className="f-row">
                  {[{v:'all',l:'Toutes'},{v:'vintage',l:'Vintage'},{v:'modern',l:'Moderne'}].map(o=>(
                    <button key={o.v} className={'f-chip'+(filterEra===o.v?' on':'')} onClick={()=>setFilterEra(o.v)}>{o.l}</button>
                  ))}
                </div>
              </div>
              <div className="f-group">
                <div className="f-label">S{'\u00e9'}rie</div>
                <div className="f-row">
                  <button className={'f-chip'+(filterSet==='all'?' on':'')} onClick={()=>setFilterSet('all')}>Toutes</button>
                  {SETS.map(st=>(
                    <button key={st} className={'f-chip'+(filterSet===st?' on':'')} onClick={()=>setFilterSet(filterSet===st?'all':st)}>{st}</button>
                  ))}
                </div>
              </div>
              <div className="f-group">
                <div className="f-label">Raret{'\u00e9'}</div>
                <div className="f-row">
                  <button className={'f-chip'+(filterRarity==='all'?' on':'')} onClick={()=>setFilterRarity('all')}>Toutes</button>
                  {RARITIES.map(r=>(
                    <button key={r} className={'f-chip'+(filterRarity===r?' on':'')} onClick={()=>setFilterRarity(filterRarity===r?'all':r)}>{r}</button>
                  ))}
                </div>
              </div>
              <div className="f-group">
                <div className="f-label">Prix</div>
                <div style={{display:'flex',alignItems:'center',gap:4}}>
                  <input className="f-price" placeholder="Min" value={filterPriceMin} onChange={e=>setFilterPriceMin(e.target.value.replace(/[^0-9]/g,''))}/>
                  <span style={{fontSize:10,color:'#CCC'}}>{'\u2192'}</span>
                  <input className="f-price" placeholder="Max" value={filterPriceMax} onChange={e=>setFilterPriceMax(e.target.value.replace(/[^0-9]/g,''))}/>
                  <span style={{fontSize:9,color:'#CCC'}}>{'\u20ac'}</span>
                </div>
              </div>
              <div className="f-group">
                <div className="f-label">Tendance</div>
                <div className="f-row">
                  {[{v:'all',l:'Toutes'},{v:'hot',l:FIRE+' Hot (+15%)'},{v:'up',l:'\u25b2 Hausse'},{v:'down',l:'\u25bc Baisse'},{v:'stable',l:'\u2194 Stable'}].map(o=>(
                    <button key={o.v} className={'f-chip'+(filterTrend===o.v?' on':'')} onClick={()=>setFilterTrend(o.v)}>{o.l}</button>
                  ))}
                </div>
              </div>
              <div className="f-group">
                <div className="f-label">Gradation</div>
                <div className="f-row">
                  {[{v:'all',l:'Tous'},{v:'psa10+',l:'PSA 10 > 1000\u20ac'},{v:'premium',l:'Prime x2+'}].map(o=>(
                    <button key={o.v} className={'f-chip'+(filterGrade===o.v?' on':'')} onClick={()=>setFilterGrade(o.v)}>{o.l}</button>
                  ))}
                </div>
              </div>
              {activeFilters>0&&(
                <div style={{display:'flex',justifyContent:'flex-end',marginTop:4}}>
                  <button className="f-reset" onClick={resetFilters}>{'\u00d7'} R{'\u00e9'}initialiser ({activeFilters})</button>
                </div>
              )}
            </div>
          )}
          ...
          {/* Count */}
          <div style={{padding:'4px 12px 4px',fontSize:10,color:'#BBB',fontFamily:'var(--font-display)'}}>{filtered.length} carte{filtered.length>1?'s':''}</div>
