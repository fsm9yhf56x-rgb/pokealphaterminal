...
export function Holdings() {
  ...
  const resetFilters = () => {
    setFilSet('all');setFilYear('all');setFilLang('all');setFilRarity('all')
    setFilType('all');setFilGraded('all');setFilPerf('all');setFilValue('all');setFilSignal('all')
  }
  ...
  const CardArt = ({ card, size='md' }: { card:CardItem; size?:'sm'|'md'|'lg' }) => {
    ...
    return (
      <div style={{ width:'100%', height:`${h}px`, borderRadius:'10px', background:`linear-gradient(145deg,${ec}20,${ec}08)`, border:`1.5px solid ${ec}28`, display:'flex', alignItems:'center', justifyContent:'center', position:'relative', overflow:'hidden', flexShrink:0 }}>
        ...
        {card.signal && (
          <div style={{ position:'absolute', top:'6px', right:'6px', fontSize:'8px', fontWeight:700, background:card.signal==='S'?'linear-gradient(135deg,#FFD700,#FF8C00)':card.signal==='A'?'#C855D4':'#2E9E6A', color:'#fff', padding:'2px 6px', borderRadius:'4px', fontFamily:'var(--font-display)', zIndex:2 }}>{card.signal}</div>
        )}
        <button onClick={e=>toggleFav(card.id,e)} style={{ position:'absolute', top:'6px', left:'6px', background:'none', border:'none', cursor:'pointer', fontSize:'13px', padding:0, zIndex:2, lineHeight:1 }}>
          {favorites.has(card.id)?'❤️':'🤍'}
        </button>
        <div style={{ position:'absolute', bottom:'5px', left:'6px', fontSize:'8px', fontWeight:700, background:LANG_STYLE[card.lang].bg, color:LANG_STYLE[card.lang].color, border:`1px solid ${LANG_STYLE[card.lang].border}`, padding:'1px 5px', borderRadius:'3px', fontFamily:'var(--font-display)', zIndex:2 }}>
          {LANG_STYLE[card.lang].flag} {card.lang}
        </div>
        {card.graded && (
          <div style={{ position:'absolute', bottom:'5px', right:'6px', fontSize:'8px', fontWeight:700, background:'#111', color:'#fff', padding:'1px 5px', borderRadius:'3px', fontFamily:'var(--font-display)', zIndex:2 }}>{card.condition}</div>
        )}
      </div>
    )
  }

  const rows = (groupCards: CardItem[]) => grouped ? Array.from(grouped.entries()) : [['', groupCards] as [string, CardItem[]]]
  ...
  return (
    <>
      <style>{`
        ...
        .pill:hover           { border-color:#999 !important; }
        ...
        .binder-card:hover    { transform:translateY(-4px) rotate(-0.5deg) !important; box-shadow:0 12px 28px rgba(0,0,0,0.4) !important; }
      `}</style>
      ...
      <div style={{ animation:'fadeIn 0.25s ease-out', width:'100%' }}>
          ...
          <div style={{ position:'relative', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'20px' }}>
            <div>
              <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'10px' }}>
                ...
                <div>
                  <div style={{ fontSize:'18px', fontWeight:700, color:'#fff', fontFamily:'var(--font-display)', lineHeight:1 }}>Ma Collection</div>
              ...
              <div style={{ display:'flex', gap:'14px', alignItems:'center', flexWrap:'wrap' }}>
                <div>
                  ...
                  <div style={{ fontSize:'12px', color:'#4ECCA3', fontWeight:600, marginTop:'3px', fontFamily:'var(--font-display)' }}>▲ +{totalROI}% ROI · +€ {totalGain.toLocaleString('fr-FR')} gain total</div>
                </div>
                ...
                {!collapsed.has(gk) && (
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:'12px', marginBottom:gk?'24px':0 }}>
                    {gc.map((card,idx)=>{
                      ...
                      const ec=ENERGY[card.type]??'#888'
                      return (
                        <div key={card.id} className="ch" style={{ background:'#fff', border:'1px solid #EBEBEB', borderRadius:'12px', overflow:'hidden', boxShadow:'0 2px 8px rgba(0,0,0,0.05)', transition:'all 0.18s', cursor:'pointer', animation:`cardIn 0.2s ${Math.min(idx,8)*0.02}s ease-out both` }}>
        ...
        {/* ── VUE BINDER ────────────────────────────────────── */}
        {view==='binder' && (
          <div style={{ background:'#0F0A04', borderRadius:'24px', padding:'32px', boxShadow:'0 16px 48px rgba(0,0,0,0.5)', border:'2px solid #1E1208', position:'relative', overflow:'hidden' }}>
            <div style={{ position:'absolute', inset:0, backgroundImage:'radial-gradient(circle at 10% 10%, rgba(255,107,53,0.06) 0%, transparent 40%), radial-gradient(circle at 90% 90%, rgba(66,165,245,0.04) 0%, transparent 40%)', pointerEvents:'none' }} />
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'24px', paddingBottom:'18px', borderBottom:'1px solid rgba(255,255,255,0.06)', position:'relative' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                <div style={{ width:'36px', height:'36px', borderRadius:'10px', background:'linear-gradient(135deg,#FF7A5A,#E03020)', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontSize:'14px', fontWeight:700 }}>P</div>
                <div>
                  <div style={{ fontSize:'15px', fontWeight:700, color:'rgba(255,255,255,0.9)', fontFamily:'var(--font-display)' }}>Ma Collection</div>
                  <div style={{ fontSize:'10px', color:'rgba(255,255,255,0.25)', fontFamily:'var(--font-display)', letterSpacing:'0.08em', textTransform:'uppercase' }}>PokéAlpha Terminal</div>
                </div>
              </div>
              <div style={{ textAlign:'right' }}>
                <div style={{ fontSize:'22px', fontWeight:700, color:'rgba(255,255,255,0.9)', fontFamily:'var(--font-display)', letterSpacing:'-0.5px' }}>€ {totalCur.toLocaleString('fr-FR')}</div>
                <div style={{ fontSize:'11px', color:'#4ECCA3', fontWeight:600 }}>+{totalROI}% ROI</div>
              </div>
            </div>
            {(grouped ? Array.from(grouped.entries()) : [['',filtered] as [string,CardItem[]]]).map(([gk,gc])=>(
              <div key={gk} style={{ marginBottom:gk?'20px':0 }}>
                {gk && (
                  <div onClick={()=>toggleGroup(gk)} style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'14px', cursor:'pointer' }}>
                    <span style={{ fontSize:'11px', fontWeight:700, color:'rgba(255,255,255,0.5)', textTransform:'uppercase' as const, letterSpacing:'0.1em', fontFamily:'var(--font-display)' }}>{gk}</span>
                    <div style={{ flex:1, height:'1px', background:'rgba(255,255,255,0.08)' }} />
                    <span style={{ fontSize:'10px', color:'rgba(255,255,255,0.25)', fontFamily:'var(--font-display)' }}>{gc.length} cartes</span>
                    <span style={{ fontSize:'10px', color:'rgba(255,255,255,0.25)' }}>{collapsed.has(gk)?'▶':'▼'}</span>
                  </div>
                )}
                {!collapsed.has(gk) && (
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'14px' }}>
                    {gc.map((card,idx)=>{
                      ...
                      return (
                        <div key={card.id} className="binder-card" style={{ background:`linear-gradient(145deg,${ec}18,${ec}06)`, border:`1.5px solid ${ec}35`, borderRadius:'14px', padding:'12px', cursor:'pointer', transition:'all 0.2s cubic-bezier(0.34,1.2,0.64,1)', position:'relative', overflow:'hidden', animation:`cardIn 0.25s ${Math.min(idx,8)*0.03}s ease-out both`, boxShadow:'0 4px 12px rgba(0,0,0,0.3)' }}>
                          <div style={{ position:'absolute', top:0, left:0, right:0, height:'2.5px', background:`linear-gradient(90deg,${ec},${ec}44)` }} />
                          <div style={{ width:'100%', height:'68px', borderRadius:'8px', background:`${ec}15`, display:'flex', alignItems:'center', justifyContent:'center', marginBottom:'10px', position:'relative', overflow:'hidden' }}>
                            <div style={{ position:'absolute', width:'50%', height:'50%', borderRadius:'50%', background:ENERGY_GLOW[card.type]??'transparent', filter:'blur(10px)' }} />
                            <div style={{ width:'28px', height:'28px', borderRadius:'50%', background:ec, opacity:0.4, zIndex:1 }} />
                            {card.signal && <div style={{ position:'absolute', top:'4px', right:'4px', fontSize:'7px', fontWeight:700, background:card.signal==='S'?'#FFD700':card.signal==='A'?'#C855D4':'#2E9E6A', color:'#fff', padding:'1px 5px', borderRadius:'3px', fontFamily:'var(--font-display)', zIndex:2 }}>{card.signal}</div>}
                            {favorites.has(card.id) && <div style={{ position:'absolute', top:'4px', left:'4px', fontSize:'10px', zIndex:2 }}>❤️</div>}
                          </div>
                          <div style={{ fontSize:'11px', fontWeight:600, color:'rgba(255,255,255,0.88)', fontFamily:'var(--font-display)', marginBottom:'2px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{card.name}</div>
                          <div style={{ fontSize:'9px', color:'rgba(255,255,255,0.28)', marginBottom:'8px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{card.set}</div>
                          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                            <div>
                              <div style={{ fontSize:'13px', fontWeight:700, color:'rgba(255,255,255,0.9)', fontFamily:'var(--font-display)' }}>€ {card.curPrice}</div>
                              <span style={{ fontSize:'7px', background:ls.bg, color:ls.color, padding:'1px 4px', borderRadius:'2px', fontWeight:700, fontFamily:'var(--font-display)' }}>{ls.flag} {card.lang}</span>
                            </div>
                            <div style={{ fontSize:'12px', fontWeight:700, color:roi>=0?'#4ECCA3':'#FF6B8A' }}>{roi>=0?'+':''}{roi}%</div>
                          </div>
                        </div>
                      )
                    })}
                ...
                {!collapsed.has(gk) && gc.map((card,i)=>{
                  ...
                  return (
                    <div key={card.id} className="rh" onClick={()=>setSelected(sel?null:card.id)} style={{ display:'grid', gridTemplateColumns:'2.4fr 0.8fr 0.9fr 1fr 1fr 90px', padding:'13px 16px', borderBottom:i<gc.length-1?'1px solid #F8F8F8':'none', background:sel?'#FFFDE0':'transparent', alignItems:'center', borderLeft:sel?`3px solid ${ec}`:'3px solid transparent', transition:'all 0.1s' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                        ...
                        <div style={{ minWidth:0 }}>
                          ...
                          <div style={{ display:'flex', alignItems:'center', gap:'4px', flexWrap:'wrap' }}>
                            ...
                            {rs && <span style={{ fontSize:'9px', background:rs.bg, color:rs.color, border:`1px solid ${rs.border}`, padding:'1px 5px', borderRadius:'3px', fontWeight:500, fontFamily:'var(--font-display)' }}>{card.rarity}</span>}
                      ...
                      <div style={{ textAlign:'right', fontSize:'13px', color:'#888', fontFamily:'var(--font-display)' }}>€ {(card.buyPrice*card.qty).toLocaleString('fr-FR')}</div>
                      <div style={{ textAlign:'right' }}>
