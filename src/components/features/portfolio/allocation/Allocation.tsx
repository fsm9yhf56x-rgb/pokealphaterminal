...
function computeAlerts(ctx: {
  ...
  if (ctx.topCardPct > 40) {
    alerts.push({
      ...
      message: `Une seule carte représente ${(ctx.topCardPct ?? 0).toFixed(0)}% du portefeuille. Diversification fortement recommandée.`,
    })
  } else if (ctx.top5Pct > 70) {
    alerts.push({
      level: 'warn',
      title: 'Top 5 cartes dominent',
      message: `Les 5 premières cartes représentent ${(ctx.top5Pct ?? 0).toFixed(0)}% du portefeuille.`,
    })
  }
  ...
  for (const [dim, buckets] of [
    ...
    if (top && top.pct > 60 && top.label !== 'N/A') {
      alerts.push({
        ...
        message: `${(top.pct ?? 0).toFixed(0)}% du portefeuille en "${top.label}".`,
      })
    }
  }
