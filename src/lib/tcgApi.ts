...
export async function fetchSets(lang:Lang): Promise<TCGSet[]> {
  const l   = LC[lang]
  const key = `tcg_sets_${l}`
  const hit = getCache<TCGSet[]>(key)
  if (hit) return hit
...
export async function fetchCardsForSet(lang:Lang, setId:string): Promise<TCGCard[]> {
  const l   = LC[lang]
  const key = `tcg_cards_${l}_${setId}`
  const hit = getCache<TCGCard[]>(key)
  if (hit) return hit
  ...
  const cards: TCGCard[] = (raw.cards ?? []).map(c => ({
    id:      c.id ?? c.localId,
    name:    c.name,
    localId: c.localId,
    image:   c.image,
...
export async function searchCards(lang:Lang, query:string): Promise<TCGCard[]> {
  const l   = LC[lang]
  const key = `tcg_search_${l}_${query.toLowerCase()}`
  const hit = getCache<TCGCard[]>(key)
  if (hit) return hit
